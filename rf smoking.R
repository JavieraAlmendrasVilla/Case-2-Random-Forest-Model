...
rf_train <- randomForest(## Define formula and data
  ...
  maxnodes = 100, # Maximum number of leaf nodes
  mtry = 17, # Number of feature variables as candidates for each split
  sampsize=length(smoking_train_dt),
)
...
ggroc <- ggplot(smoking_test_dt, aes(d=as.numeric(Smoking), m=preds_rf)) +
  geom_roc() +
...
smoking_dt <- rbindlist(list(smoking_train_dt, smoking_test_dt))    # Rbind data.tables
